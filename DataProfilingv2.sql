...
/* -------------------------------------------------------------------
Project: All DW Projects

Process: Data Profiling

Description: Process to profile the data in a specific table.
----------------------------------------------------------------------
Change Audit
Version When       Who             Why
1.1     06/09/2019 Tim Stowe       Add Specific Functionality 
----------------------------------------------------------------------- */
...
-- Define the table you want to profile
DECLARE	@TABLE_SCHEMA NVARCHAR(128) = 'dbo'         -- Your schema here
DECLARE	@TABLE_NAME NVARCHAR(128) = 'DW_Account'        -- Your table here
...
DECLARE	@UICol BIT = 1								-- Include the 
...
-- FACTMULTIAP SPECIFIC PARAMETERS
-- If you are profiling Fact Multi AP use the paramaters below for a sample size
DECLARE @APSampleRowSize INT = 10000000                -- Row Count sample size you want to analyse (if 0 if will use the AccountNumber)
DECLARE @APSampleAccount varchar(500) = ''                  -- Add the account numbers you want to sample (in the format of 'ACCNO1', 'ACCNO2')

-----------------------------------------------------------------------
-- Process variables
-----------------------------------------------------------------------

DECLARE @fullTableName varchar(128) = @TABLE_SCHEMA + '.' + @TABLE_NAME
...
BEGIN  -- Use all column names, to avoid filtering
	...
	IF @LOBCol = 1
		...
		INSERT INTO		#ColumnList (COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH)
		SELECT			COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS
		WHERE			DATA_TYPE IN ('varchar', 'nvarchar', 'varbinary', 'xml', 'binary')
						AND TABLE_NAME = @TABLE_NAME
						AND TABLE_SCHEMA = @TABLE_SCHEMA
	...
	IF @UICol = 1
		BEGIN
		INSERT INTO		#ColumnList (COLUMN_NAME, DATA_TYPE, CHARACTER_MAXIMUM_LENGTH)
		SELECT			COLUMN_NAME, DATA_TYPE, 32 FROM INFORMATION_SCHEMA.COLUMNS
		WHERE			DATA_TYPE IN ('uniqueidentifier')
						AND TABLE_NAME = @TABLE_NAME
						AND TABLE_SCHEMA = @TABLE_SCHEMA
		END
...
SET @ROWCOUNTTEXT = 'SELECT @ROWCOUNTOUT = COUNT (*) FROM ' + @fullTableName + ' WITH (NOLOCK)'
...
-----------------------------------------------------------------------
	...
	,NULL 
	,NULL 
	,(SELECT COUNT (*) from ' + @fullTableName + ' WHERE ISNUMERIC(' + QUOTENAME(@COLUMN_NAME) + ') = 1) 
	,(SELECT COUNT (*) from ' + @fullTableName + ' WHERE ISDATE(' + QUOTENAME(@COLUMN_NAME) + ') = 1) '
	...
	,MAX(' + QUOTENAME(@COLUMN_NAME) + ') 
	,MIN(' + QUOTENAME(@COLUMN_NAME) + ')
	,(SELECT COUNT (*) from ' + @fullTableName + ' WHERE (CONVERT(NUMERIC(20,12), ' + QUOTENAME(@COLUMN_NAME) + ' ) - FLOOR(CONVERT(NUMERIC(20,12), ' + QUOTENAME(@COLUMN_NAME) + ')) <> 0))
	,(SELECT COUNT (*) from ' + @fullTableName + ' WHERE DATEPART(ss,' + QUOTENAME(@COLUMN_NAME) + ') <> 0 OR DATEPART(mcs,' + QUOTENAME(@COLUMN_NAME) + ') <> 0)
	,NULL 
	,NULL '
	...
	,NULL 
	,NULL '
	WHEN @DATA_TYPE IN ('binary', 'uniqueidentifier') THEN + '
	,MAX(LEN(' + QUOTENAME(@COLUMN_NAME) + '))  
	,MIN(LEN(' + QUOTENAME(@COLUMN_NAME) + '))  
	,AVG(LEN(' + QUOTENAME(@COLUMN_NAME) + '))
	,NULL
	...
	,NULL '
	END	+ '
	,(SELECT COUNT(*) FROM ' + @fullTableName + ' WHERE ' + QUOTENAME(@COLUMN_NAME) + ' IS NULL)'
	+ CASE
	WHEN @DATA_TYPE IN ('varchar', 'nvarchar', 'char', 'nchar') THEN + '
	,(SELECT COUNT(*) FROM ' + @fullTableName + ' WHERE LEN(LTRIM(RTRIM(' + QUOTENAME(@COLUMN_NAME) + '))) = '''')'
	ELSE + '
	,NULL'
	END	+ '
	,(SELECT COUNT(DISTINCT ' + QUOTENAME(@COLUMN_NAME) + ') FROM ' + @fullTableName + ' WHERE ' + QUOTENAME(@COLUMN_NAME) + ' IS NOT NULL) 
...
-----------------------------------------------------------------------
	...
	END
	FROM		#ProfileData 
	WHERE		DataType IN ('numeric', 'int', 'bigint', 'tinyint', 'smallint', 'decimal', 'money', 'smallmoney', 'float','real','varchar', 'nvarchar', 'char', 'nchar', 'binary', 'uniqueidentifier')
